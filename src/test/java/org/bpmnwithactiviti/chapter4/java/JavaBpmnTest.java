...
public class JavaBpmnTest {
	...
	private ProcessInstance startProcessInstance() {
		...
		return runtimeService.startProcessInstanceByKey("bookorder",
				variableMap);
	}
	...
	public void executeAsyncService() {
		JobExecutor jobExecutor = ((ProcessEngineImpl) activitiRule
	...
	private boolean waitForJobExecutorToProcessAllJobs(long maxMillisToWait,
			long intervalMillis) {
	...
	public boolean areJobsAvailable() {
		return !activitiRule.getManagementService().createJobQuery()
				.executable().list().isEmpty();
	}
